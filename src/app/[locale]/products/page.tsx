...
export default async function ProductsPage({ params: { locale } }: PageProps) {
  ...
  return (
    <>
      {/* Hero Section */}
      <section className="relative min-h-[600px] md:min-h-[700px] bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
                {t('pageTitle')}{' '}
                <span className="text-brand-purple">{t('pageTitleHighlight')}</span>
              </h1>
              <p className="text-xl text-gray-300 mb-8 leading-relaxed">{t('pageSubtitle')}</p>
            ...
            {featuredProduct && (
              <div>
                ...
                >
                  ...
                  <p className="text-gray-600 mb-6 line-clamp-3">{featuredProduct.excerpt}</p>
        ...
        <div className="container-custom">
          <div className="max-w-4xl mx-auto text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-text-primary mb-4">
              {t('whatAreTitle')} <span className="text-brand-purple">{t('whatAreTitleHighlight')}</span>?
            </h2>
            <p className="text-lg text-text-secondary">{t('whatAreSubtitle')}</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: t('card1Title'), desc: t('card1Desc'), icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z', color: 'brand-purple' },
              { title: t('card2Title'), desc: t('card2Desc'), icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10', color: 'brand-blue' },
              { title: t('card3Title'), desc: t('card3Desc'), icon: 'M13 10V3L4 14h7v7l9-11h-7z', color: 'brand-purple' },
            ].map((card, idx) => (
              <div key={idx} className="bg-white rounded-xl p-8 shadow-soft hover:shadow-medium transition-shadow">
                <div className={`w-12 h-12 bg-${card.color}/10 rounded-lg flex items-center justify-center mb-4`}>
                  <svg className={`w-6 h-6 text-${card.color}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <section className="section-padding bg-background-secondary">
        <div className="container-custom">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-text-primary mb-6">{t('noCodingTitle')}</h2>
              <p className="text-lg text-text-secondary mb-8">{t('noCodingSubtitle')}</p>
              <div className="space-y-4">
                {[
                  { title: t('noCodingSetup'), desc: t('noCodingSetupDesc') },
                  { title: t('noCodingUI'), desc: t('noCodingUIDesc') },
                  { title: t('noCodingDocs'), desc: t('noCodingDocsDesc') },
                ].map((item, idx) => (
          ...
          <div className="text-center mb-12">
            <div className="mb-6 flex justify-center">
              <div className="pill-wrapper">
                <div className="pill section-pill center">{t('pill')}</div>
              </div>
            </div>
            <h2 className="text-3xl md:text-4xl font-bold text-text-primary mb-4">
              {t('sectionTitle')} <span className="text-brand-purple">{t('sectionTitleHighlight')}</span>
            </h2>
            <p className="text-lg text-text-secondary max-w-2xl mx-auto">{t('sectionSubtitle')}</p>
          </div>
        </div>
        <ProductsList />
      </section>
