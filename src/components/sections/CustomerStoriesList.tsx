        ...
        <div ref={cardsRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 min-h-[400px]">
          {customerStories.map((story) => (
            <Link key={story.id} href={`/customers/${story.slug}`} className="group relative overflow-hidden rounded-xl bg-white shadow-soft hover:shadow-medium transition-all duration-300">
              ...
              <div className="p-6">
                <h3 className="text-lg font-semibold text-text-primary mb-2 group-hover:text-brand-purple transition-colors">{story.title}</h3>
                <p className="text-sm text-text-secondary mb-4 line-clamp-2">{story.excerpt}</p>
