...
export default function Button({
  ...
  if (href) {
    return (
      <Link href={href} className={`${classes} hover:scale-105`} style={{ transition: 'transform 0.2s ease' }}>
        {children}
      </Link>
    )
  }
  ...
  return (
    <button className={`${classes} hover:scale-105`} style={{ transition: 'transform 0.2s ease' }} {...props}>
      {children}
    </button>
  )
}
